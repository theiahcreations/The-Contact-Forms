function createIAHFormCorrected() {
  try {
    ...
    var sheetIdMatch = sheetUrl.match(/\/d\/([a-zA-Z0-9-_]+)/);

    if (sheetIdMatch && sheetIdMatch[1]) {
      var sheetId = sheetIdMatch[1];
      var sheet = SpreadsheetApp.openById(sheetId);
      form.setDestination(FormApp.DestinationType.SPREADSHEET, sheet.getId());
    } else {
      console.error("Invalid Sheet URL. Could not set destination.");
    }
    ...
    // We create the item first, then we will add choices with navigation later
